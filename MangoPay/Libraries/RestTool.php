...
class RestTool
{
    ...
    private function BuildRequest($urlMethod, $pagination, $additionalUrlParams = null, $idempotencyKey = null)
    {
        ...
        if ($this->_root->Config->ProxyHost != null && $this->_root->Config->ProxyPort != null) {
            curl_setopt($this->_curlHandle, CURLOPT_PROXY, $this->_root->Config->ProxyHost);
            curl_setopt($this->_curlHandle, CURLOPT_PROXYPORT, $this->_root->Config->ProxyPort);

            if ($this->_root->Config->ProxyUser != null && $this->_root->Config->ProxyPassword != null) {
                dump(sprintf('%s:%s', $this->_root->Config->ProxyUser, $this->_root->Config->ProxyPassword));
                curl_setopt($this->_curlHandle, CURLOPT_PROXYUSERPWD, sprintf('%s:%s', $this->_root->Config->ProxyUser, $this->_root->Config->ProxyPassword));
            }
        }
